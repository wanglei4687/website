...
团队成员应该通过阅读概述来决定这个设计是否有阅读和理解的必要，最多为3段
...
如果里程碑的预计到达时间（ETA：Estimated Time of Arrival）发生了变化，添加一个`[update]`的小节来对项目相关的人员查看和重新估计
...
尝试通过用户故事来具化解决方案的目标，尽量（随意）添加子部分和图表。
...
这里说一点所有这些都是确保在程序失败的时候提供追踪的方法和路径（即使是flaky test），也是确定软件的
边界/系统边界。
...
如果在没有互联网访问的情况下，团队中的某个人可以更具文档并按照设计意图实施吗？
...
来验证想法，为确保只编写探索性的代码，取保原型代码的中任何一个提交都不会合并到主要分支。
